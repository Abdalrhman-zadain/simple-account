...
- `phase-4-procure-to-pay/purchases` now includes Supplier master records, Purchase Requests (draft/submit/approve/reject/close, request lines, status history, and request-to-order conversion), and Purchase Orders (draft/issue/partial receipt/full receipt/cancel/close, direct creation, and request-linked creation); purchase invoices, supplier payments, debit notes, and payable posting flows are still pending.
