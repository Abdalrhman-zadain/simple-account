...
- `phase-4-procure-to-pay/purchases` now includes Supplier master records, Purchase Requests (draft/submit/approve/reject/close, request lines, status history, and request-to-order conversion), Purchase Orders (draft/issue/partial receipt/full receipt/cancel/close, direct creation, and request-linked creation), Purchase Invoice drafts (direct/order-linked creation, line account classification, discounts/tax/totals, and draft editing), Supplier Payments (draft capture, invoice allocation, posting through Bank & Cash, cancellation, and supplier-balance reduction), and Debit Notes (draft/edit/post/cancel, optional purchase-invoice linkage, supplier-balance reduction, and payable-balance reduction on linked invoices). Purchase-invoice and debit-note journal posting flows are still pending.
