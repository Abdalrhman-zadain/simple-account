...
export function MasterDataPage() {
    ...
    if (isLoading) return <div className="flex items-center justify-center py-40 text-gray-500">Loading master data...</div>;
                ...
                <Card className="p-0 border border-gray-200 bg-panel/40  overflow-hidden">
                    ...
                    <div className="flex items-center justify-between border-b border-gray-200 px-6 py-5">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">{activeDef.description || `Manage ${activeDef.name} master values`}</p>
                        </div>
                    ...
                    {showAdd && (
                        <div className="border-b border-gray-200 px-6 py-4 bg-teal-500/5">
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => setShowAdd(false)} className="p-2 text-gray-500 hover:text-gray-900">
                                    <X className="h-4 w-4" />
                                </button>
                            </div>
                        </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="border-b border-gray-200 bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-600">Code</th>
                                <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-600">Name</th>
                                <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-600 text-center">Status</th>
                                <th className="px-6 py-3 text-right text-[10px] font-bold uppercase tracking-widest text-gray-600">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {activeDef.values.length === 0 ? (
                                <tr><td colSpan={4} className="px-6 py-12 text-center text-sm text-gray-600">No values yet. Add one above.</td></tr>
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            {editingId === val.id ? (
                                                <>
                                                    ...
                                                    <button onClick={() => setEditingId(null)} className="p-1.5 rounded-lg text-gray-500 hover:text-gray-900">
                                                        <X className="h-4 w-4" />
                                                    </button>
                                                </>
                                            ) : (
                                                <>
                                                    <button onClick={() => { setEditingId(val.id); setEditCode(val.code); setEditName(val.name); }} className="p-1.5 rounded-lg text-gray-500 hover:text-teal-400 hover:bg-teal-400/10 transition-all">
                                                        <Pencil className="h-4 w-4" />
                                                    </button>
                                                    {val.isActive && (
                                                        <button onClick={() => { if (confirm(`Deactivate "${val.name}"?`)) deactivateMutation.mutate(val.id); }} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-400/10 transition-all">
                                                            <X className="h-4 w-4" />
                                                        </button>
                                                    )}
                                                </>
