    ...
    <div className="space-y-6">
      <Card className="border border-gray-200 bg-white p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-lg font-bold text-gray-900">{selectedDetails?.name ?? t("bankCash.title")}</div>
        ...
        {selectedDetails ? (
          ...
          <div className="mt-6 text-sm text-gray-500">{t("bankCash.history.description")}</div>
        )}
      </Card>

      <Card className="overflow-hidden border border-gray-200 bg-panel/40 p-0">
        <div className="border-b border-gray-200 bg-gray-50 px-6 py-4">
          <div className="text-sm font-bold text-gray-900">{t("bankCash.history.title")}</div>
          <div className="text-xs text-gray-500">{t("bankCash.history.description")}</div>
        </div>
        <div className="max-h-[620px] overflow-auto">
          {!selectedId ? (
            <div className="px-6 py-10 text-sm text-gray-500">{t("bankCash.history.description")}</div>
          ) : isLoading ? (
            <div className="px-6 py-10 text-sm text-gray-500">{t("common.loading")}</div>
          ) : historyRows.length === 0 ? (
            <div className="px-6 py-10 text-sm text-gray-500">{t("bankCash.history.empty")}</div>
          ) : (
              ...
              <tbody className="divide-y divide-white/5">
                {historyRows.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50">
                    <td className="px-6 py-3 text-xs text-gray-500">{formatDate(row.entryDate)}</td>
