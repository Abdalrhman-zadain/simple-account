        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            {showAdd && (
                <Card className="border border-teal-500/20 bg-teal-500/5 p-5">
                    <div className="flex items-center gap-4">
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-1.5">Year</label>
                            <input
                                type="number"
                                value={newYear}
                                onChange={e => setNewYear(e.target.value)}
                                placeholder="e.g. 2026"
                                className="w-32 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-teal-500/40"
                            />
                        </div>
                        <div className="pt-6">
                            <Button
            ...
            <div className="space-y-4">
                {yearsQuery.isLoading ? (
                    <div className="py-20 text-center text-sm text-zinc-600">Loading fiscal years...</div>
                ) : years.length === 0 ? (
                    <div className="py-20 text-center text-sm text-zinc-600">No fiscal years yet. Create one above.</div>
                    ...
                    return (
                        <Card key={year.id} className="p-0 border border-white/5 bg-panel/40 backdrop-blur-xl overflow-hidden">
                            <div
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-bold text-white">Fiscal Year {year.year}</h3>
                                        <p className="text-xs text-zinc-500">{formatDate(year.startDate)} → {formatDate(year.endDate)}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-xs text-zinc-600">{openPeriods} open / {year.periods.length} periods</span>
                                ...
                                <div className="border-t border-white/5">
                                    <table className="w-full text-sm">
                                        <thead className="bg-white/[0.02] border-b border-white/5">
                                            <tr>
                                                <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-600">#</th>
                                                <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-600">Period</th>
                                                <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-zinc-600">Date Range</th>
                                                <th className="px-6 py-3 text-center text-[10px] font-bold uppercase tracking-widest text-zinc-600">Status</th>
                                                <th className="px-6 py-3 text-right text-[10px] font-bold uppercase tracking-widest text-zinc-600">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-white/5">
                                            {year.periods.map((period: FiscalPeriod) => (
                                                <tr key={period.id} className="hover:bg-white/[0.02] transition-colors">
                                                    <td className="px-6 py-3 text-xs font-mono text-zinc-600">{String(period.periodNumber).padStart(2, "0")}</td>
                                                    ...
                                                    <td className="px-6 py-3 text-right">
                                                        {period.status === "OPEN" && (
                                                            <button
                                                                ...
                                                                className="flex items-center gap-1.5 ml-auto rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-bold text-zinc-400 hover:text-red-400 hover:border-red-400/20 hover:bg-red-400/10 transition-all"
                                                            >
