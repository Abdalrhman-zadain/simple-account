...
describe('PostingService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    journalEntriesService.validateLines.mockReset();
    journalEntriesService.validateLines.mockImplementation(() => undefined);
  ...
  it('rejects posting when a line targets a header account', async () => {
    tx.journalEntry.findUnique.mockResolvedValue({
      id: 'entry-3',
      reference: 'JE-20260409-HEADER',
      status: JournalEntryStatus.DRAFT,
      entryDate: new Date('2026-04-09T00:00:00.000Z'),
      description: 'Header account entry',
      lines: [
        {
          id: 'line-1',
          accountId: 'assets',
          description: 'Debit assets',
          lineNumber: 1,
          debitAmount: 100,
          creditAmount: 0,
        },
    ...
    tx.account.findMany.mockResolvedValue([
      { id: 'assets', isActive: true, isPosting: false, allowManualPosting: true },
      { id: 'revenue', isActive: true, isPosting: true, allowManualPosting: true },
    ]);

    await expect(service.post('entry-3')).rejects.toThrow(
      'Account assets is a header account and cannot receive journal entries.',
    );
